#ifndef AES128TABLES_H
#define AES128TABLES_H

#include <cstdint>

/**
 * @namespace Aes128Tables
 * @brief Lookup tables for AES-128 algorithm optimization
 */
namespace Aes128Tables
{
/**
 * @brief Forward S-Box
 */
const uint8_t kSbox[256] = {
    };

/**
 * @brief combined Xtimes2[Sbox[]]
 */
const uint8_t kXtimes2Sbox[256] = {
    };

/**
 * @brief combined Xtimes3[Sbox[]]
 */
const uint8_t kXtimes3Sbox[256] = {
    };

/**
 * @brief Inverse S-box
 */
const uint8_t kInvSbox[256] = {
    };

/**
 * @brief X^9 Modular multiplication table
 */
const uint8_t kXtimes9[256] = {
    };

/**
 * @brief X^B Modular multiplication table
 */
const uint8_t kXtimesB[256] = {
    };

/**
 * @brief X^D Modular multiplication table
 */
const uint8_t kXtimesD[256] = {
    };

/**
 * @brief X^E Modular multiplication table
 */
const uint8_t kXtimesE[256] = {
    };
}  // namespace Aes128Tables

#endif  // AES128TABLES_H
